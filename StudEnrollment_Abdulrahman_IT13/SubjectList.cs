...
{
    public class SubjectList : Form
    {
        private DataGridView dgvSubjects;
        private TextBox txtSearch;
        private Button btnAdd, btnEdit, btnDelete, btnRefresh;
        private Label lblHeader, lblTotalSubjects;
        ...
        private void InitializeComponent()
        {
            ...
            btnAdd = CreateModernButton("➕ Add New Subject", Color.FromArgb(34, 197, 94), 0);
            btnEdit = CreateModernButton("✏️ Edit Subject", Color.FromArgb(59, 130, 246), 180);
            ...
            dgvSubjects.ColumnHeadersDefaultCellStyle.BackColor = Color.FromArgb(249, 250, 251);
            dgvSubjects.ColumnHeadersDefaultCellStyle.ForeColor = Color.FromArgb(55, 65, 81);
            dgvSubjects.ColumnHeadersDefaultCellStyle.Font = new Font("Segoe UI Semibold", 10, FontStyle.Bold);
            dgvSubjects.ColumnHeadersDefaultCellStyle.Padding = new Padding(10, 8, 10, 8);
            dgvSubjects.ColumnHeadersDefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;
            dgvSubjects.ColumnHeadersHeight = 45;
            dgvSubjects.EnableHeadersVisualStyles = false;

            // Modern row styling
            dgvSubjects.RowsDefaultCellStyle.BackColor = Color.White;
            dgvSubjects.RowsDefaultCellStyle.Font = new Font("Segoe UI", 9.5F);
            dgvSubjects.RowsDefaultCellStyle.ForeColor = Color.FromArgb(55, 65, 81);
            dgvSubjects.RowsDefaultCellStyle.Padding = new Padding(10, 5, 10, 5);
            dgvSubjects.RowTemplate.Height = 42;
            dgvSubjects.AlternatingRowsDefaultCellStyle.BackColor = Color.FromArgb(249, 250, 251);
            dgvSubjects.DefaultCellStyle.SelectionBackColor = Color.FromArgb(219, 234, 254);
            dgvSubjects.DefaultCellStyle.SelectionForeColor = Color.FromArgb(30, 64, 175);
            dgvSubjects.CellBorderStyle = DataGridViewCellBorderStyle.SingleHorizontal;
            dgvSubjects.GridColor = Color.FromArgb(229, 231, 235);

            dgvCard.Controls.Add(dgvSubjects);
        ...
        private void LoadSubjects()
        {
            try
            {
                ...
                if (dgvSubjects.Columns.Count > 0)
                {
                    ...
                    dgvSubjects.Columns["SubjectCode"].Width = 140;
                    ...
                    dgvSubjects.Columns["Units"].HeaderText = "⏱️ Units";
                    dgvSubjects.Columns["Units"].Width = 100;

                    dgvSubjects.Columns["Department"].HeaderText = "🏢 Department";
                    dgvSubjects.Columns["Department"].Width = 150;

                    dgvSubjects.Columns["Instructor"].HeaderText = "👨‍🏫 Instructor";
                    dgvSubjects.Columns["Instructor"].Width = 200;
                }
        ...
        private void BtnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                bool success = DatabaseHelper.AddSubject(
                    dialog.SubjectCode,
                    dialog.Description,
                    dialog.Units,
                    dialog.Department,
        ...
        private void BtnEdit_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("⚠️ Please select a subject to edit!", "No Selection",
            ...
            string instructor = row.Cells["Instructor"].Value != DBNull.Value ?
                row.Cells["Instructor"].Value.ToString() : "";

            int units = Convert.ToInt32(row.Cells["Units"].Value);

            SubjectDialog dialog = new SubjectDialog(
                row.Cells["SubjectCode"].Value.ToString(),
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                bool success = DatabaseHelper.UpdateSubject(
                    dialog.SubjectCode,
                    dialog.Description,
                    dialog.Units,
                    dialog.Department,
        ...
        private void BtnDelete_Click(object sender, EventArgs e)
        {
            if (dgvSubjects.SelectedRows.Count == 0)
            {
                MessageBox.Show("⚠️ Please select a subject to delete!", "No Selection",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            string subjectCode = dgvSubjects.SelectedRows[0].Cells["SubjectCode"].Value.ToString();
            string description = dgvSubjects.SelectedRows[0].Cells["Description"].Value.ToString();
            ...
            {
                bool success = DatabaseHelper.DeleteSubject(subjectCode);
                if (success)
                {
        ...
        {
            txtSearch.Clear();
            txtSearch.Text = "Search subjects...";
        ...
        private Panel CreateSearchPanel()
        {
            ...
            txtSearch.Enter += (s, e) =>
            {
                if (txtSearch.Text == "Search subjects...")
                {
                    txtSearch.Text = "";
            ...
            {
                if (txtSearch.Text != "Search subjects..." && !string.IsNullOrWhiteSpace(txtSearch.Text))
                {
                    try
                    {
                        DataTable dt = DatabaseHelper.SearchSubjects(txtSearch.Text);
                        dgvSubjects.DataSource = dt;
                        lblTotalSubjects.Text = dt.Rows.Count.ToString();
                    }
                    catch (Exception ex)
                ...
                else if (string.IsNullOrWhiteSpace(txtSearch.Text) || txtSearch.Text == "Search subjects...")
                {
                    LoadSubjects();
                }
            };
        ...
        private Panel CreateStatCard()
        {
            ...
            card.Controls.Add(lblTotalSubjects);
